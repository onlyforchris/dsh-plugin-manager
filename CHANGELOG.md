...
- **独立左侧菜单「插件管家」**：从「插件」菜单下的 tab 提升为设置页一级菜单（`settings.section`，order 26，位于「插件」之后），拥有完整内容区与品牌入口；「插件」菜单回归 DSH 原生职责（配置/列表）。
- **顶部概览卡（Hero）**：一眼看清管家价值——已验证 / 实验性 / 可升级 / 结构健康 / 环境自检统计、Profile、目录来源与管家版本；「关于」与「运行环境自检」底部面板并入 Hero 后移除。
- **手动安装（高级）**：改名降调为折叠项，明确适用场景（不在推荐目录中的信任插件）。
- **视觉收敛（克制原生风）**：遵循 `docs/ui-standards.md` 通用 UI 标准——无装饰色条、无渐变；状态用小徽章区分（✓ 已验证 / 实验性 / 社区收录）、首次使用步骤编号圆点、元信息徽章化、入场动画与 hover 提升、健康点呼吸动画；颜色全部走宿主设计令牌。
